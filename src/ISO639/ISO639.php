<?php
/**
 * Created in LeEco
 * User: Marlin<toleranthink@gmail.com>
 * Date: 4/29/16
 * Time: 11:12 PM
 * @copyright LeEco
 * @since 1.0.0
 */

namespace L10n\ISO639;


use DomainException;
use IteratorAggregate;
use OutOfBoundsException;

class ISO639 implements IteratorAggregate, DataProvider
{
    const KEY_EN_NAME = 'english_name';
    const KEY_HTTP_NAME = 'http_name';
    const KEY_ISO639_V2 = 'iso_639_2';
    const KEY_ISO639_V1 = 'iso_639_1';

    /**
     * @param $englishName
     * @return mixed
     */
    public function getByEnglishName($englishName)
    {
        if (!preg_match(
            '/^[a-zA-Z]*?$/',
            $englishName
        )
        ) {
            throw new DomainException('Not a valid English Name: ' . $englishName);
        }
        return self::get($englishName);
    }

    /**
     * @param $httpName
     * @return mixed
     */
    public function getByHttpName($httpName)
    {
        if (!preg_match(
            '/^[a-zA-Z]{2}-[a-zA-Z]{2}$/',
            $httpName
        )
        ) {
            throw new DomainException('Not a valid Http Name: ' . $httpName);
        }
        return self::get($httpName);
    }

    /**
     * @param $iso639V1
     * @return mixed
     */
    public function getByISO639V1($iso639V1)
    {
        if (!preg_match(
            '/^[a-z]{2}$/',
            $iso639V1
        )
        ) {
            throw new DomainException('Not a valid ISO 639-1 name' . $iso639V1);
        }
        return self::get($iso639V1);
    }

    /**
     * @param $iso639V2
     * @return mixed
     */
    public function getByISO639V2($iso639V2)
    {
        if (!preg_match(
            '/^[a-z]{3}$/',
            $iso639V2
        )
        ) {
            throw new DomainException('Not a valid ISO 639-2 name' . $iso639V2);
        }
        return self::get($iso639V2);
    }

    /**
     * @api
     * @return array
     */
    public function getAll()
    {
        return $this->languages;
    }

    /**
     * @param string $listBy
     * @return \Generator
     */
    public function listBy($listBy = self::KEY_HTTP_NAME)
    {
        if (!in_array(
            $listBy,
            $keys = [
                self::KEY_ISO639_V2,
                self::KEY_ISO639_V1,
                self::KEY_EN_NAME,
                self::KEY_HTTP_NAME
            ],
            true
        )
        ) {
            throw new DomainException(sprintf('Invalid value for $indexBy, got "%s", expected one of: %s', $listBy,
                                              implode(', ', $keys)));
        }
        foreach ($this->languages as $language) {
            yield $language[$listBy] => $language;
        }
    }

    public function isValid($id){
        return $this->get($id);
    }
    /**
     * @internal
     * @return \Generator
     */
    public function getIterator()
    {
        foreach ($this->languages as $language) {
            yield $language;
        }
    }

    /**
     * @param $id
     * @return mixed
     */
    public function get($id)
    {
        foreach ($this as $language) {
            if (0 === strcasecmp(
                    $id,
                    $language[self::KEY_EN_NAME]
                ) || 0 === strcasecmp(
                    $id,
                    $language[self::KEY_HTTP_NAME]
                ) || 0 === strcasecmp(
                    $id,
                    $language[self::KEY_ISO639_V1]
                ) || 0 === strcasecmp(
                    $id,
                    $language[self::KEY_ISO639_V2]
                )

            ) {
                return $language;
            }
        }
        throw new OutOfBoundsException('ISO 639 does not contain: ' . $id);
    }

    protected $languages = [
        'aa'    => [
            'english_name' => 'Afar',
            'native_name'  => 'Afaraf',
            'is_rtl'       => 0,
            'iso_639_1'    => 'aa',
            'iso_639_2'    => 'aar',
            'http_name'    => 'aa',
            'priority'     => 1,
        ],
        'ae'    => [
            'english_name' => 'Avestan',
            'native_name'  => 'avesta',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ae',
            'iso_639_2'    => 'ave',
            'http_name'    => 'ae',
            'priority'     => 1,
        ],
        'af'    => [
            'english_name' => 'Afrikaans',
            'native_name'  => 'Afrikaans',
            'is_rtl'       => 0,
            'iso_639_1'    => 'af',
            'iso_639_2'    => 'afr',
            'http_name'    => 'af-ZA',
            'priority'     => 1,
        ],
        'ak'    => [
            'english_name' => 'Akan',
            'native_name'  => 'Akan',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ak',
            'iso_639_2'    => 'aka',
            'http_name'    => 'ak',
            'priority'     => 1,
        ],
        'am'    => [
            'english_name' => 'Amharic',
            'native_name'  => 'አማርኛ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'am',
            'iso_639_2'    => 'amh',
            'http_name'    => 'am',
            'priority'     => 1,
        ],
        'an'    => [
            'english_name' => 'Aragonese',
            'native_name'  => 'Aragonés',
            'is_rtl'       => 0,
            'iso_639_1'    => 'an',
            'iso_639_2'    => 'arg',
            'http_name'    => 'an',
            'priority'     => 1,
        ],
        'ar'    => [
            'english_name' => 'Arabic',
            'native_name'  => 'العربية',
            'is_rtl'       => 1,
            'iso_639_1'    => 'ar',
            'iso_639_2'    => 'ara',
            'http_name'    => 'ar-AR',
            'priority'     => 1,
        ],
        'as'    => [
            'english_name' => 'Assamese',
            'native_name'  => 'অসমীয়া',
            'is_rtl'       => 0,
            'iso_639_1'    => 'as',
            'iso_639_2'    => 'asm',
            'http_name'    => 'asm',
            'priority'     => 1,
        ],
        'ast'   => [
            'english_name' => 'Asturian',
            'native_name'  => 'Asturianu',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => 'ast',
            'http_name'    => 'ast',
            'priority'     => 1,
        ],
        'av'    => [
            'english_name' => 'Avaric',
            'native_name'  => 'авар мацӀ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'av',
            'iso_639_2'    => 'ava',
            'http_name'    => 'av',
            'priority'     => 1,
        ],
        'ay'    => [
            'english_name' => 'Aymara',
            'native_name'  => 'aymar aru',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ay',
            'iso_639_2'    => 'aym',
            'http_name'    => 'ay',
            'priority'     => 1,
        ],
        'az'    => [
            'english_name' => 'Azerbaijani',
            'native_name'  => 'Azərbaycan dili',
            'is_rtl'       => 0,
            'iso_639_1'    => 'az',
            'iso_639_2'    => 'aze',
            'http_name'    => 'az-AZ',
            'priority'     => 1,
        ],
        'azb'   => [
            'english_name' => 'South Azerbaijani',
            'native_name'  => 'گؤنئی آذربایجان',
            'is_rtl'       => 1,
            'iso_639_1'    => 'az',
            'iso_639_2'    => 'aze',
            'http_name'    => 'az',
            'priority'     => 1,
        ],
        'az-tr' => [
            'english_name' => 'Azerbaijani (Turkey)',
            'native_name'  => 'Azərbaycan Türkcəsi',
            'is_rtl'       => 1,
            'iso_639_1'    => 'az',
            'iso_639_2'    => 'aze',
            'http_name'    => 'az-TR',
            'priority'     => 1,
        ],
        'ba'    => [
            'english_name' => 'Bashkir',
            'native_name'  => 'башҡорт теле',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ba',
            'iso_639_2'    => 'bak',
            'http_name'    => 'ba',
            'priority'     => 1,
        ],
        'bal'   => [
            'english_name' => 'Catalan (Balear)',
            'native_name'  => 'Català (Balear)',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ca',
            'iso_639_2'    => 'cat',
            'http_name'    => 'ca',
            'priority'     => 1,
        ],
        'bel'   => [
            'english_name' => 'Belarusian',
            'native_name'  => 'Беларуская мова',
            'is_rtl'       => 0,
            'iso_639_1'    => 'be',
            'iso_639_2'    => 'bel',
            'http_name'    => 'be-BY',
            'priority'     => 1,
        ],
        'bg'    => [
            'english_name' => 'Bulgarian',
            'native_name'  => 'Български',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bg',
            'iso_639_2'    => 'bul',
            'http_name'    => 'bg-BG',
            'priority'     => 1,
        ],
        'bh'    => [
            'english_name' => 'Bihari',
            'native_name'  => 'भोजपुरी',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bh',
            'iso_639_2'    => 'bih',
            'http_name'    => 'bh',
            'priority'     => 1,
        ],
        'bi'    => [
            'english_name' => 'Bislama',
            'native_name'  => 'Bislama',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bi',
            'iso_639_2'    => 'bis',
            'http_name'    => 'bi',
            'priority'     => 1,
        ],
        'bm'    => [
            'english_name' => 'Bambara',
            'native_name'  => 'Bamanankan',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bm',
            'iso_639_2'    => 'bam',
            'http_name'    => 'bm',
            'priority'     => 1,
        ],
        'bn'    => [
            'english_name' => 'Bengali',
            'native_name'  => 'বাংলা',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bn',
            'iso_639_2'    => 'ben',
            'http_name'    => 'bn-BD',
            'priority'     => 1,
        ],
        'bo'    => [
            'english_name' => 'Tibetan',
            'native_name'  => 'བོད་སྐད',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bo',
            'iso_639_2'    => 'bod',
            'http_name'    => 'bo',
            'priority'     => 1,
        ],
        'br'    => [
            'english_name' => 'Breton',
            'native_name'  => 'brezhoneg',
            'is_rtl'       => 0,
            'iso_639_1'    => 'br',
            'iso_639_2'    => 'bre',
            'http_name'    => 'br',
            'priority'     => 1,
        ],
        'bs'    => [
            'english_name' => 'Bosnian',
            'native_name'  => 'Bosanski',
            'is_rtl'       => 0,
            'iso_639_1'    => 'bs',
            'iso_639_2'    => 'bos',
            'http_name'    => 'bs-BA',
            'priority'     => 1,
        ],
        'ca'    => [
            'english_name' => 'Catalan',
            'native_name'  => 'Català',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ca',
            'iso_639_2'    => 'cat',
            'http_name'    => 'ca-ES',
            'priority'     => 1,
        ],
        'ce'    => [
            'english_name' => 'Chechen',
            'native_name'  => 'Нохчийн мотт',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ce',
            'iso_639_2'    => 'che',
            'http_name'    => 'ce',
            'priority'     => 1,
        ],
        'ch'    => [
            'english_name' => 'Chamorro',
            'native_name'  => 'Chamoru',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ch',
            'iso_639_2'    => 'cha',
            'http_name'    => 'ch',
            'priority'     => 1,
        ],
        'ckb'   => [
            'english_name' => 'Kurdish (Sorani)',
            'native_name'  => 'كوردی‎',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ku',
            'iso_639_2'    => 'kur',
            'http_name'    => 'ku',
            'priority'     => 1,
        ],
        'co'    => [
            'english_name' => 'Corsican',
            'native_name'  => 'corsu',
            'is_rtl'       => 0,
            'iso_639_1'    => 'co',
            'iso_639_2'    => 'cos',
            'http_name'    => 'co',
            'priority'     => 1,
        ],
        'cr'    => [
            'english_name' => 'Cree',
            'native_name'  => 'ᓀᐦᐃᔭᐍᐏᐣ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'cr',
            'iso_639_2'    => 'cre',
            'http_name'    => 'cr',
            'priority'     => 1,
        ],
        'cs'    => [
            'english_name' => 'Czech',
            'native_name'  => 'čeština‎',
            'is_rtl'       => 0,
            'iso_639_1'    => 'cs',
            'iso_639_2'    => 'cze',
            'http_name'    => 'cs-CZ',
            'priority'     => 1,
        ],
        'csb'   => [
            'english_name' => 'Kashubian',
            'native_name'  => 'Kaszëbsczi',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => 'csb',
            'http_name'    => 'csb',
            'priority'     => 1,
        ],
        'cu'    => [
            'english_name' => 'Church Slavic',
            'native_name'  => 'ѩзыкъ словѣньскъ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'cu',
            'iso_639_2'    => 'chu',
            'http_name'    => 'cu',
            'priority'     => 1,
        ],
        'cv'    => [
            'english_name' => 'Chuvash',
            'native_name'  => 'чӑваш чӗлхи',
            'is_rtl'       => 0,
            'iso_639_1'    => 'cv',
            'iso_639_2'    => 'chv',
            'http_name'    => 'cv',
            'priority'     => 1,
        ],
        'cy'    => [
            'english_name' => 'Welsh',
            'native_name'  => 'Cymraeg',
            'is_rtl'       => 0,
            'iso_639_1'    => 'cy',
            'iso_639_2'    => 'cym',
            'http_name'    => 'cy-GB',
            'priority'     => 1,
        ],
        'da'    => [
            'english_name' => 'Danish',
            'native_name'  => 'Dansk',
            'is_rtl'       => 0,
            'iso_639_1'    => 'da',
            'iso_639_2'    => 'dan',
            'http_name'    => 'da-DK',
            'priority'     => 1,
        ],
        'de'    => [
            'english_name' => 'German',
            'native_name'  => 'Deutsch',
            'is_rtl'       => 0,
            'iso_639_1'    => 'de',
            'iso_639_2'    => 'deu',
            'http_name'    => 'de-DE',
            'priority'     => 1,
        ],
        'dv'    => [
            'english_name' => 'Divehi',
            'native_name'  => 'ދިވެހި',
            'is_rtl'       => 1,
            'iso_639_1'    => 'dv',
            'iso_639_2'    => 'div',
            'http_name'    => 'dv',
            'priority'     => 1,
        ],
        'dz'    => [
            'english_name' => 'Dzongkha',
            'native_name'  => 'རྫོང་ཁ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'dz',
            'iso_639_2'    => 'dzo',
            'http_name'    => 'dz',
            'priority'     => 1,
        ],
        'ee'    => [
            'english_name' => 'Ewe',
            'native_name'  => 'Eʋegbe',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ee',
            'iso_639_2'    => 'ewe',
            'http_name'    => 'ee',
            'priority'     => 1,
        ],
        'el-po' => [
            'english_name' => 'Greek (Polytonic)',
            'native_name'  => 'Greek (Polytonic)',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => 'grc',
            'http_name'    => 'grc',
            'priority'     => 1,
        ],
        'el'    => [
            'english_name' => 'Greek',
            'native_name'  => 'Ελληνικά',
            'is_rtl'       => 0,
            'iso_639_1'    => 'el',
            'iso_639_2'    => 'ell',
            'http_name'    => 'el-GR',
            'priority'     => 1,
        ],
        'en'    => [
            'english_name' => 'English',
            'native_name'  => 'English',
            'is_rtl'       => 0,
            'iso_639_1'    => 'en',
            'iso_639_2'    => 'eng',
            'http_name'    => 'en-US',
            'priority'     => 1,
        ],
        'en-au' => [
            'english_name' => 'English (Australia)',
            'native_name'  => 'English (Australia)',
            'is_rtl'       => 0,
            'iso_639_1'    => 'en',
            'iso_639_2'    => 'eng',
            'http_name'    => 'en-AU',
            'priority'     => 1,
        ],
        'en-ca' => [
            'english_name' => 'English (Canada)',
            'native_name'  => 'English (Canada)',
            'is_rtl'       => 0,
            'iso_639_1'    => 'en',
            'iso_639_2'    => 'eng',
            'http_name'    => 'en-CA',
            'priority'     => 1,
        ],
        'en-gb' => [
            'english_name' => 'English (UK)',
            'native_name'  => 'English (UK)',
            'is_rtl'       => 0,
            'iso_639_1'    => 'en',
            'iso_639_2'    => 'eng',
            'http_name'    => 'en-GB',
            'priority'     => 1,
        ],
        'en-us' => [
            'english_name' => 'English (US)',
            'native_name'  => 'English (US)',
            'is_rtl'       => 0,
            'iso_639_1'    => 'en',
            'iso_639_2'    => 'eng',
            'http_name'    => 'en-US',
            'priority'     => 1,
        ],
        'eo'    => [
            'english_name' => 'Esperanto',
            'native_name'  => 'Esperanto',
            'is_rtl'       => 0,
            'iso_639_1'    => 'eo',
            'iso_639_2'    => 'epo',
            'http_name'    => 'eo-EO',
            'priority'     => 1,
        ],
        'es-cl' => [
            'english_name' => 'Spanish (Chile)',
            'native_name'  => 'Español de Chile',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-CL',
            'priority'     => 1,
        ],
        'es-mx' => [
            'english_name' => 'Spanish (Mexico)',
            'native_name'  => 'Español de México',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-MX',
            'priority'     => 1,
        ],
        'es-pe' => [
            'english_name' => 'Spanish (Peru)',
            'native_name'  => 'Español de Perú',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-PE',
            'priority'     => 1,
        ],
        'es-pr' => [
            'english_name' => 'Spanish (Puerto Rico)',
            'native_name'  => 'Español de Puerto Rico',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-PR',
            'priority'     => 1,
        ],
        'es-ve' => [
            'english_name' => 'Spanish (Venezuela)',
            'native_name'  => 'Español de Venezuela',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-VE',
            'priority'     => 1,
        ],
        'es-co' => [
            'english_name' => 'Spanish (Colombia)',
            'native_name'  => 'Español de Colombia',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-CO',
            'priority'     => 1,
        ],
        'es'    => [
            'english_name' => 'Spanish (Spain)',
            'native_name'  => 'Español',
            'is_rtl'       => 0,
            'iso_639_1'    => 'es',
            'iso_639_2'    => 'spa',
            'http_name'    => 'es-ES',
            'priority'     => 1,
        ],
        'et'    => [
            'english_name' => 'Estonian',
            'native_name'  => 'Eesti',
            'is_rtl'       => 0,
            'iso_639_1'    => 'et',
            'iso_639_2'    => 'est',
            'http_name'    => 'et-EE',
            'priority'     => 1,
        ],
        'eu'    => [
            'english_name' => 'Basque',
            'native_name'  => 'Euskara',
            'is_rtl'       => 0,
            'iso_639_1'    => 'eu',
            'iso_639_2'    => 'eus',
            'http_name'    => 'eu-ES',
            'priority'     => 1,
        ],
        'fa'    => [
            'english_name' => 'Persian',
            'native_name'  => 'فارسی',
            'is_rtl'       => 1,
            'iso_639_1'    => 'fa',
            'iso_639_2'    => 'fas',
            'http_name'    => 'fa-IR',
            'priority'     => 1,
        ],
        'fa-af' => [
            'english_name' => 'Persian (Afghanistan)',
            'native_name'  => '(فارسی (افغانستان',
            'is_rtl'       => 1,
            'iso_639_1'    => 'fa',
            'iso_639_2'    => 'eus',
            'http_name'    => 'fa-AF',
            'priority'     => 1,
        ],
        'fi'    => [
            'english_name' => 'Finnish',
            'native_name'  => 'Suomi',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fi',
            'iso_639_2'    => 'fin',
            'http_name'    => 'fi-FI',
            'priority'     => 1,
        ],
        'fj'    => [
            'english_name' => 'Fijian',
            'native_name'  => 'vosa Vakaviti',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fj',
            'iso_639_2'    => 'fij',
            'http_name'    => 'fj',
            'priority'     => 1,
        ],
        'fo'    => [
            'english_name' => 'Faroese',
            'native_name'  => 'føroyskt',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fo',
            'iso_639_2'    => 'fao',
            'http_name'    => 'fo-FO',
            'priority'     => 1,
        ],
        'fr'    => [
            'english_name' => 'French (France)',
            'native_name'  => 'Français',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fr',
            'iso_639_2'    => 'fra',
            'http_name'    => 'fr-FR',
            'priority'     => 1,
        ],
        'fr-be' => [
            'english_name' => 'French (Belgium)',
            'native_name'  => 'Français de Belgique',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fr',
            'iso_639_2'    => 'fra',
            'http_name'    => 'fr-BE',
            'priority'     => 1,
        ],
        'fr-ca' => [
            'english_name' => 'French (Canada)',
            'native_name'  => 'Français du Canada',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fr',
            'iso_639_2'    => 'fra',
            'http_name'    => 'fr-CA',
            'priority'     => 1,
        ],
        'fr-ch' => [
            'english_name' => 'French (Switzerland)',
            'native_name'  => 'Français de Suisse',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fr',
            'iso_639_2'    => 'fra',
            'http_name'    => 'fr',
            'priority'     => 1,
        ],
        'fy'    => [
            'english_name' => 'Frisian',
            'native_name'  => 'Frysk',
            'is_rtl'       => 0,
            'iso_639_1'    => 'fy',
            'iso_639_2'    => 'fry',
            'http_name'    => 'fy-NL',
            'priority'     => 1,
        ],
        'ga'    => [
            'english_name' => 'Irish',
            'native_name'  => 'Gaelige',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ga',
            'iso_639_2'    => 'gle',
            'http_name'    => 'ga-IE',
            'priority'     => 1,
        ],
        'gd'    => [
            'english_name' => 'Scottish Gaelic',
            'native_name'  => 'Gàidhlig',
            'is_rtl'       => 0,
            'iso_639_1'    => 'gd',
            'iso_639_2'    => 'gla',
            'http_name'    => 'gd',
            'priority'     => 1,
        ],
        'gl'    => [
            'english_name' => 'Galician',
            'native_name'  => 'Galego',
            'is_rtl'       => 0,
            'iso_639_1'    => 'gl',
            'iso_639_2'    => 'glg',
            'http_name'    => 'gl-ES',
            'priority'     => 1,
        ],
        'gn'    => [
            'english_name' => 'Guaraní',
            'native_name'  => 'Avañe’ẽ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'gn',
            'iso_639_2'    => 'grn',
            'http_name'    => 'gn',
            'priority'     => 1,
        ],
        'gsw'   => [
            'english_name' => 'Swiss German',
            'native_name'  => 'Schwyzerdütsch',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'gu'    => [
            'english_name' => 'Gujarati',
            'native_name'  => 'ગુજરાતી',
            'is_rtl'       => 0,
            'iso_639_1'    => 'gu',
            'iso_639_2'    => 'guj',
            'http_name'    => 'gu',
            'priority'     => 1,
        ],
        'ha'    => [
            'english_name' => 'Hausa',
            'native_name'  => 'هَوُسَ',
            'is_rtl'       => 1,
            'iso_639_1'    => 'ha',
            'iso_639_2'    => 'hau',
            'http_name'    => 'ha',
            'priority'     => 1,
        ],
        'haw'   => [
            'english_name' => 'Hawaiian',
            'native_name'  => 'Ōlelo Hawaiʻi',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => 'haw-US',
            'priority'     => 1,
        ],
        'haz'   => [
            'english_name' => 'Hazaragi',
            'native_name'  => 'هزاره گی',
            'is_rtl'       => 1,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'he'    => [
            'english_name' => 'Hebrew',
            'native_name'  => 'עִבְרִית',
            'is_rtl'       => 1,
            'iso_639_1'    => 'he',
            'iso_639_2'    => 'heb',
            'http_name'    => 'he-IL',
            'priority'     => 1,
        ],
        'hi'    => [
            'english_name' => 'Hindi',
            'native_name'  => 'हिन्दी',
            'is_rtl'       => 0,
            'iso_639_1'    => 'hi',
            'iso_639_2'    => 'hin',
            'http_name'    => 'hi-IN',
            'priority'     => 1,
        ],
        'hr'    => [
            'english_name' => 'Croatian',
            'native_name'  => 'Hrvatski',
            'is_rtl'       => 0,
            'iso_639_1'    => 'hr',
            'iso_639_2'    => 'hrv',
            'http_name'    => 'hr-HR',
            'priority'     => 1,
        ],
        'hu'    => [
            'english_name' => 'Hungarian',
            'native_name'  => 'Magyar',
            'is_rtl'       => 0,
            'iso_639_1'    => 'hu',
            'iso_639_2'    => 'hun',
            'http_name'    => 'hu-HU',
            'priority'     => 1,
        ],
        'hy'    => [
            'english_name' => 'Armenian',
            'native_name'  => 'Հայերեն',
            'is_rtl'       => 0,
            'iso_639_1'    => 'hy',
            'iso_639_2'    => 'hye',
            'http_name'    => 'hy-AM',
            'priority'     => 1,
        ],
        'ia'    => [
            'english_name' => 'Interlingua',
            'native_name'  => 'Interlingua',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ia',
            'iso_639_2'    => 'ina',
            'http_name'    => 'ia',
            'priority'     => 1,
        ],
        'id'    => [
            'english_name' => 'Indonesian',
            'native_name'  => 'Bahasa Indonesia',
            'is_rtl'       => 0,
            'iso_639_1'    => 'id',
            'iso_639_2'    => 'ind',
            'http_name'    => 'id-ID',
            'priority'     => 1,
        ],
        'ike'   => [
            'english_name' => 'Inuktitut',
            'native_name'  => 'ᐃᓄᒃᑎᑐᑦ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'iu',
            'iso_639_2'    => 'iku',
            'http_name'    => 'iu',
            'priority'     => 1,
        ],
        'ilo'   => [
            'english_name' => 'Iloko',
            'native_name'  => 'Pagsasao nga Iloko',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'is'    => [
            'english_name' => 'Icelandic',
            'native_name'  => 'Íslenska',
            'is_rtl'       => 0,
            'iso_639_1'    => 'is',
            'iso_639_2'    => 'isl',
            'http_name'    => 'is-IS',
            'priority'     => 1,
        ],
        'it'    => [
            'english_name' => 'Italian',
            'native_name'  => 'Italiano',
            'is_rtl'       => 0,
            'iso_639_1'    => 'it',
            'iso_639_2'    => 'ita',
            'http_name'    => 'it-IT',
            'priority'     => 1,
        ],
        'ja'    => [
            'english_name' => 'Japanese',
            'native_name'  => '日本語',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ja',
            'iso_639_2'    => 'jpn',
            'http_name'    => 'ja-JP',
            'priority'     => 1,
        ],
        'jv'    => [
            'english_name' => 'Javanese',
            'native_name'  => 'Basa Jawa',
            'is_rtl'       => 0,
            'iso_639_1'    => 'jv',
            'iso_639_2'    => 'jav',
            'http_name'    => 'jv-ID',
            'priority'     => 1,
        ],
        'ka'    => [
            'english_name' => 'Georgian',
            'native_name'  => 'ქართული',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ka',
            'iso_639_2'    => 'kat',
            'http_name'    => 'ka-GE',
            'priority'     => 1,
        ],
        'rw'    => [
            'english_name' => 'Kinyarwanda',
            'native_name'  => 'Kinyarwanda',
            'is_rtl'       => 0,
            'iso_639_1'    => 'rw',
            'iso_639_2'    => 'kin',
            'http_name'    => 'rw',
            'priority'     => 1,
        ],
        'kk'    => [
            'english_name' => 'Kazakh',
            'native_name'  => 'Қазақ тілі',
            'is_rtl'       => 0,
            'iso_639_1'    => 'kk',
            'iso_639_2'    => 'kaz',
            'http_name'    => 'kk',
            'priority'     => 1,
        ],
        'km'    => [
            'english_name' => 'Khmer',
            'native_name'  => 'ភាសាខ្មែរ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'km',
            'iso_639_2'    => 'khm',
            'http_name'    => 'km-KH',
            'priority'     => 1,
        ],
        'kn'    => [
            'english_name' => 'Kannada',
            'native_name'  => 'ಕನ್ನಡ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'kn',
            'iso_639_2'    => 'kan',
            'http_name'    => 'kn',
            'priority'     => 1,
        ],
        'ko'    => [
            'english_name' => 'Korean',
            'native_name'  => '한국어',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ko',
            'iso_639_2'    => 'kor',
            'http_name'    => 'ko-KR',
            'priority'     => 1,
        ],
        'ks'    => [
            'english_name' => 'Kashmiri',
            'native_name'  => 'कश्मीरी',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ks',
            'iso_639_2'    => 'kas',
            'http_name'    => 'ks',
            'priority'     => 1,
        ],
        'ku'    => [
            'english_name' => 'Kurdish (Kurmanji)',
            'native_name'  => 'Kurdî',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ku',
            'iso_639_2'    => 'kur',
            'http_name'    => 'ku-TR',
            'priority'     => 1,
        ],
        'ky'    => [
            'english_name' => 'Kyrgyz',
            'native_name'  => 'кыргыз тили',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ky',
            'iso_639_2'    => 'kir',
            'http_name'    => 'ky-KY',
            'priority'     => 1,
        ],
        'la'    => [
            'english_name' => 'Latin',
            'native_name'  => 'latine',
            'is_rtl'       => 0,
            'iso_639_1'    => 'la',
            'iso_639_2'    => 'lat',
            'http_name'    => 'la-VA',
            'priority'     => 1,
        ],
        'lb'    => [
            'english_name' => 'Luxembourgish',
            'native_name'  => 'Lëtzebuergesch',
            'is_rtl'       => 0,
            'iso_639_1'    => 'lb',
            'iso_639_2'    => 'ltz',
            'http_name'    => 'lb-LU',
            'priority'     => 1,
        ],
        'li'    => [
            'english_name' => 'Limburgish',
            'native_name'  => 'Limburgs',
            'is_rtl'       => 0,
            'iso_639_1'    => 'li',
            'iso_639_2'    => 'lim',
            'http_name'    => 'li',
            'priority'     => 1,
        ],
        'lo'    => [
            'english_name' => 'Lao',
            'native_name'  => 'ພາສາລາວ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'lo',
            'iso_639_2'    => 'lao',
            'http_name'    => 'lo',
            'priority'     => 1,
        ],
        'lt'    => [
            'english_name' => 'Lithuanian',
            'native_name'  => 'lietuvių kalba',
            'is_rtl'       => 0,
            'iso_639_1'    => 'lt',
            'iso_639_2'    => 'lit',
            'http_name'    => 'lt-LT',
            'priority'     => 1,
        ],
        'lv'    => [
            'english_name' => 'Latvian',
            'native_name'  => 'latviešu valoda',
            'is_rtl'       => 0,
            'iso_639_1'    => 'lv',
            'iso_639_2'    => 'lav',
            'http_name'    => 'lv-LV',
            'priority'     => 1,
        ],
        'me'    => [
            'english_name' => 'Montenegrin',
            'native_name'  => 'Crnogorski jezik',
            'is_rtl'       => 0,
            'iso_639_1'    => 'me',
            'iso_639_2'    => '',
            'http_name'    => 'me-ME',
            'priority'     => 1,
        ],
        'mg'    => [
            'english_name' => 'Malagasy',
            'native_name'  => 'Malagasy',
            'is_rtl'       => 0,
            'iso_639_1'    => 'mg',
            'iso_639_2'    => 'mlg',
            'http_name'    => 'mg-MG',
            'priority'     => 1,
        ],
        'mhr'   => [
            'english_name' => 'Mari (Meadow)',
            'native_name'  => 'олык марий',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'mk'    => [
            'english_name' => 'Macedonian',
            'native_name'  => 'македонски јазик',
            'is_rtl'       => 0,
            'iso_639_1'    => 'mk',
            'iso_639_2'    => 'mkd',
            'http_name'    => 'mk-MK',
            'priority'     => 1,
        ],
        'ml'    => [
            'english_name' => 'Malayalam',
            'native_name'  => 'മലയാളം',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ml',
            'iso_639_2'    => 'mal',
            'http_name'    => 'ml-IN',
            'priority'     => 1,
        ],
        'mn'    => [
            'english_name' => 'Mongolian',
            'native_name'  => 'Монгол',
            'is_rtl'       => 0,
            'iso_639_1'    => 'mn',
            'iso_639_2'    => 'mon',
            'http_name'    => 'mn',
            'priority'     => 1,
        ],
        'mr'    => [
            'english_name' => 'Marathi',
            'native_name'  => 'मराठी',
            'is_rtl'       => 0,
            'iso_639_1'    => 'mr',
            'iso_639_2'    => 'mar',
            'http_name'    => 'mr',
            'priority'     => 1,
        ],
        'mri'   => [
            'english_name' => 'Maori',
            'native_name'  => 'te reo Māori',
            'is_rtl'       => 0,
            'iso_639_1'    => 'mi',
            'iso_639_2'    => 'mri',
            'http_name'    => 'mi',
            'priority'     => 1,
        ],
        'mrj'   => [
            'english_name' => 'Mari (Hill)',
            'native_name'  => 'кырык мары',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'ms'    => [
            'english_name' => 'Malay',
            'native_name'  => 'bahasa Melayu',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ms',
            'iso_639_2'    => 'msa',
            'http_name'    => 'ms-MY',
            'priority'     => 1,
        ],
        'mwl'   => [
            'english_name' => 'Mirandese',
            'native_name'  => 'Mirandés',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'mya'   => [
            'english_name' => 'Burmese',
            'native_name'  => 'ဗမာစာ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'my',
            'iso_639_2'    => 'mya',
            'http_name'    => 'my-MM',
            'priority'     => 1,
        ],
        'ne'    => [
            'english_name' => 'Nepali',
            'native_name'  => 'नेपाली',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ne',
            'iso_639_2'    => 'nep',
            'http_name'    => 'ne-NP',
            'priority'     => 1,
        ],
        'nb'    => [
            'english_name' => 'Norwegian (Bokmål)',
            'native_name'  => 'Norsk bokmål',
            'is_rtl'       => 0,
            'iso_639_1'    => 'nb',
            'iso_639_2'    => 'nob',
            'http_name'    => 'nb-NO',
            'priority'     => 1,
        ],
        'nl'    => [
            'english_name' => 'Dutch',
            'native_name'  => 'Nederlands',
            'is_rtl'       => 0,
            'iso_639_1'    => 'nl',
            'iso_639_2'    => 'nld',
            'http_name'    => 'nl-NL',
            'priority'     => 1,
        ],
        'nl-be' => [
            'english_name' => 'Dutch (Belgium)',
            'native_name'  => 'Nederlands (België)',
            'is_rtl'       => 0,
            'iso_639_1'    => 'nl',
            'iso_639_2'    => 'nld',
            'http_name'    => 'nl-BE',
            'priority'     => 1,
        ],
        'nn'    => [
            'english_name' => 'Norwegian (Nynorsk)',
            'native_name'  => 'Norsk nynorsk',
            'is_rtl'       => 0,
            'iso_639_1'    => 'nn',
            'iso_639_2'    => 'nno',
            'http_name'    => 'nn-NO',
            'priority'     => 1,
        ],
        'no'    => [
            'english_name' => 'Norwegian',
            'native_name'  => 'Norsk',
            'is_rtl'       => 0,
            'iso_639_1'    => 'no',
            'iso_639_2'    => 'nor',
            'http_name'    => 'no',
            'priority'     => 1,
        ],
        'oc'    => [
            'english_name' => 'Occitan',
            'native_name'  => 'Occitan',
            'is_rtl'       => 0,
            'iso_639_1'    => 'oc',
            'iso_639_2'    => 'oci',
            'http_name'    => 'oc',
            'priority'     => 1,
        ],
        'os'    => [
            'english_name' => 'Ossetic',
            'native_name'  => 'Ирон',
            'is_rtl'       => 0,
            'iso_639_1'    => 'os',
            'iso_639_2'    => 'oss',
            'http_name'    => 'os',
            'priority'     => 1,
        ],
        'pa'    => [
            'english_name' => 'Panjabi',
            'native_name'  => 'ਪੰਜਾਬੀ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'pa',
            'iso_639_2'    => 'pan',
            'http_name'    => 'pa-IN',
            'priority'     => 1,
        ],
        'pl'    => [
            'english_name' => 'Polish',
            'native_name'  => 'Polski',
            'is_rtl'       => 0,
            'iso_639_1'    => 'pl',
            'iso_639_2'    => 'pol',
            'http_name'    => 'pl-PL',
            'priority'     => 1,
        ],
        'pt-br' => [
            'english_name' => 'Portuguese (Brazil)',
            'native_name'  => 'Português do Brasil',
            'is_rtl'       => 0,
            'iso_639_1'    => 'pt',
            'iso_639_2'    => 'por',
            'http_name'    => 'pt-BR',
            'priority'     => 1,
        ],
        'pt'    => [
            'english_name' => 'Portuguese (Portugal)',
            'native_name'  => 'Português',
            'is_rtl'       => 0,
            'iso_639_1'    => 'pt',
            'iso_639_2'    => 'por',
            'http_name'    => 'pt-PT',
            'priority'     => 1,
        ],
        'ps'    => [
            'english_name' => 'Pashto',
            'native_name'  => 'پښتو',
            'is_rtl'       => 1,
            'iso_639_1'    => 'ps',
            'iso_639_2'    => 'pus',
            'http_name'    => 'ps-AF',
            'priority'     => 1,
        ],
        'ro'    => [
            'english_name' => 'Romanian',
            'native_name'  => 'Română',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ro',
            'iso_639_2'    => 'ron',
            'http_name'    => 'ro-RO',
            'priority'     => 1,
        ],
        'ru'    => [
            'english_name' => 'Russian',
            'native_name'  => 'Русский',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ru',
            'iso_639_2'    => 'rus',
            'http_name'    => 'ru-RU',
            'priority'     => 1,
        ],
        'ru-ua' => [
            'english_name' => 'Russian (Ukraine)',
            'native_name'  => 'украї́нська мо́ва',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ru',
            'iso_639_2'    => 'rus',
            'http_name'    => 'ru-UA',
            'priority'     => 1,
        ],
        'rue'   => [
            'english_name' => 'Rusyn',
            'native_name'  => 'Русиньскый',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'rup'   => [
            'english_name' => 'Aromanian',
            'native_name'  => 'Armãneashce',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => 'rup',
            'http_name'    => 'rup-MK',
            'priority'     => 1,
        ],
        'sah'   => [
            'english_name' => 'Yakut',
            'native_name'  => 'Sakha',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => 'sah',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'sa-in' => [
            'english_name' => 'Sanskrit',
            'native_name'  => 'भारतम्',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sa',
            'iso_639_2'    => 'san',
            'http_name'    => 'sa-IN',
            'priority'     => 1,
        ],
        'sd'    => [
            'english_name' => 'Sindhi',
            'native_name'  => 'سندھ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sd',
            'iso_639_2'    => 'snd',
            'http_name'    => 'sd-PK',
            'priority'     => 1,
        ],
        'si'    => [
            'english_name' => 'Sinhala',
            'native_name'  => 'සිංහල',
            'is_rtl'       => 0,
            'iso_639_1'    => 'si',
            'iso_639_2'    => 'sin',
            'http_name'    => 'si-LK',
            'priority'     => 1,
        ],
        'sk'    => [
            'english_name' => 'Slovak',
            'native_name'  => 'Slovenčina',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sk',
            'iso_639_2'    => 'slk',
            'http_name'    => 'sk-SK',
            'priority'     => 1,
        ],
        'sl'    => [
            'english_name' => 'Slovenian',
            'native_name'  => 'slovenščina',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sl',
            'iso_639_2'    => 'slv',
            'http_name'    => 'sl-SI',
            'priority'     => 1,
        ],
        'so'    => [
            'english_name' => 'Somali',
            'native_name'  => 'Afsoomaali',
            'is_rtl'       => 0,
            'iso_639_1'    => 'so',
            'iso_639_2'    => 'som',
            'http_name'    => 'so-SO',
            'priority'     => 1,
        ],
        'sq'    => [
            'english_name' => 'Albanian',
            'native_name'  => 'Shqip',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sq',
            'iso_639_2'    => 'sqi',
            'http_name'    => 'sq-AL',
            'priority'     => 1,
        ],
        'sr'    => [
            'english_name' => 'Serbian',
            'native_name'  => 'Српски језик',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sr',
            'iso_639_2'    => 'srp',
            'http_name'    => 'sr-RS',
            'priority'     => 1,
        ],
        'srd'   => [
            'english_name' => 'Sardinian',
            'native_name'  => 'sardu',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sc',
            'iso_639_2'    => 'srd',
            'http_name'    => 'sc',
            'priority'     => 1,
        ],
        'su'    => [
            'english_name' => 'Sundanese',
            'native_name'  => 'Basa Sunda',
            'is_rtl'       => 0,
            'iso_639_1'    => 'su',
            'iso_639_2'    => 'sun',
            'http_name'    => 'su-ID',
            'priority'     => 1,
        ],
        'sv'    => [
            'english_name' => 'Swedish',
            'native_name'  => 'Svenska',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sv',
            'iso_639_2'    => 'swe',
            'http_name'    => 'sv-SE',
            'priority'     => 1,
        ],
        'sw'    => [
            'english_name' => 'Swahili',
            'native_name'  => 'Kiswahili',
            'is_rtl'       => 0,
            'iso_639_1'    => 'sw',
            'iso_639_2'    => 'swa',
            'http_name'    => 'sw-KE',
            'priority'     => 1,
        ],
        'ta'    => [
            'english_name' => 'Tamil',
            'native_name'  => 'தமிழ்',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ta',
            'iso_639_2'    => 'tam',
            'http_name'    => 'ta-IN',
            'priority'     => 1,
        ],
        'ta-lk' => [
            'english_name' => 'Tamil (Sri Lanka)',
            'native_name'  => 'தமிழ்',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ta',
            'iso_639_2'    => 'tam',
            'http_name'    => 'ta-LK',
            'priority'     => 1,
        ],
        'te'    => [
            'english_name' => 'Telugu',
            'native_name'  => 'తెలుగు',
            'is_rtl'       => 0,
            'iso_639_1'    => 'te',
            'iso_639_2'    => 'tel',
            'http_name'    => 'te-IN',
            'priority'     => 1,
        ],
        'tg'    => [
            'english_name' => 'Tajik',
            'native_name'  => 'тоҷикӣ',
            'is_rtl'       => 0,
            'iso_639_1'    => 'tg',
            'iso_639_2'    => 'tgk',
            'http_name'    => 'tg',
            'priority'     => 1,
        ],
        'th'    => [
            'english_name' => 'Thai',
            'native_name'  => 'ไทย',
            'is_rtl'       => 0,
            'iso_639_1'    => 'th',
            'iso_639_2'    => 'tha',
            'http_name'    => 'th-TH',
            'priority'     => 1,
        ],
        'tlh'   => [
            'english_name' => 'Klingon',
            'native_name'  => 'TlhIngan',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => 'tlh',
            'priority'     => 1,
        ],
        'tl'    => [
            'english_name' => 'Tagalog',
            'native_name'  => 'Tagalog',
            'is_rtl'       => 0,
            'iso_639_1'    => 'tl',
            'iso_639_2'    => 'tgl',
            'http_name'    => 'tl-PH',
            'priority'     => 1,
        ],
        'tr'    => [
            'english_name' => 'Turkish',
            'native_name'  => 'Türkçe',
            'is_rtl'       => 0,
            'iso_639_1'    => 'tr',
            'iso_639_2'    => 'tur',
            'http_name'    => 'tr-TR',
            'priority'     => 1,
        ],
        'tt'    => [
            'english_name' => 'Tatar',
            'native_name'  => 'Татар теле',
            'is_rtl'       => 0,
            'iso_639_1'    => 'tt',
            'iso_639_2'    => 'tat',
            'http_name'    => 'tt-RU',
            'priority'     => 1,
        ],
        'tuk'   => [
            'english_name' => 'Turkmen',
            'native_name'  => 'Türkmençe',
            'is_rtl'       => 0,
            'iso_639_1'    => 'tk',
            'iso_639_2'    => 'tuk',
            'http_name'    => 'tk',
            'priority'     => 1,
        ],
        'tzm'   => [
            'english_name' => 'Tamazight (Central Atlas)',
            'native_name'  => 'ⵜⴰⵎⴰⵣⵉⵖⵜ',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'udm'   => [
            'english_name' => 'Udmurt',
            'native_name'  => 'удмурт кыл',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'ug'    => [
            'english_name' => 'Uyghur',
            'native_name'  => 'Uyƣurqə',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ug',
            'iso_639_2'    => 'uig',
            'http_name'    => 'ug-CN',
            'priority'     => 1,
        ],
        'uk'    => [
            'english_name' => 'Ukrainian',
            'native_name'  => 'Українська',
            'is_rtl'       => 0,
            'iso_639_1'    => 'uk',
            'iso_639_2'    => 'ukr',
            'http_name'    => 'uk-UA',
            'priority'     => 1,
        ],
        'ur'    => [
            'english_name' => 'Urdu',
            'native_name'  => 'اردو',
            'is_rtl'       => 0,
            'iso_639_1'    => 'ur',
            'iso_639_2'    => 'urd',
            'http_name'    => 'ur',
            'priority'     => 1,
        ],
        'uz'    => [
            'english_name' => 'Uzbek',
            'native_name'  => 'O‘zbekcha',
            'is_rtl'       => 0,
            'iso_639_1'    => 'uz',
            'iso_639_2'    => 'uzb',
            'http_name'    => 'uz-UZ',
            'priority'     => 1,
        ],
        'vec'   => [
            'english_name' => 'Venetian',
            'native_name'  => 'vèneta',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'vi'    => [
            'english_name' => 'Vietnamese',
            'native_name'  => 'Tiếng Việt',
            'is_rtl'       => 0,
            'iso_639_1'    => 'vi',
            'iso_639_2'    => 'vie',
            'http_name'    => 'vi-VN',
            'priority'     => 1,
        ],
        'wa'    => [
            'english_name' => 'Walloon',
            'native_name'  => 'Walon',
            'is_rtl'       => 0,
            'iso_639_1'    => 'wa',
            'iso_639_2'    => 'wln',
            'http_name'    => 'wa',
            'priority'     => 1,
        ],
        'xmf'   => [
            'english_name' => 'Mingrelian',
            'native_name'  => 'მარგალური ნინა',
            'is_rtl'       => 0,
            'iso_639_1'    => '',
            'iso_639_2'    => '',
            'http_name'    => '',
            'priority'     => 1,
        ],
        'yi'    => [
            'english_name' => 'Yiddish',
            'native_name'  => 'ייִדיש',
            'is_rtl'       => 1,
            'iso_639_1'    => 'yi',
            'iso_639_2'    => 'yid',
            'http_name'    => 'yi',
            'priority'     => 1,
        ],
        'yo'    => [
            'english_name' => 'Yorùbá',
            'native_name'  => 'èdè Yorùbá',
            'is_rtl'       => 0,
            'iso_639_1'    => 'yo',
            'iso_639_2'    => 'yor',
            'http_name'    => 'yo',
            'priority'     => 1,
        ],
        'zh-cn' => [
            'english_name' => 'Chinese (China)',
            'native_name'  => '中文',
            'is_rtl'       => 0,
            'iso_639_1'    => 'zh',
            'iso_639_2'    => 'zho',
            'http_name'    => 'zh-CN',
            'priority'     => 1,
        ],
        'zh-hk' => [
            'english_name' => 'Chinese (Hong Kong)',
            'native_name'  => '香港中文版	',
            'is_rtl'       => 0,
            'iso_639_1'    => 'zh',
            'iso_639_2'    => 'zho',
            'http_name'    => 'zh-HK',
            'priority'     => 1,
        ],
        'zh-sg' => [
            'english_name' => 'Chinese (Singapore)',
            'native_name'  => '中文',
            'is_rtl'       => 0,
            'iso_639_1'    => 'zh',
            'iso_639_2'    => 'zho',
            'http_name'    => 'zh',
            'priority'     => 1,
        ],
        'zh-tw' => [
            'english_name' => 'Chinese (Taiwan)',
            'native_name'  => '中文',
            'is_rtl'       => 0,
            'iso_639_1'    => 'zh',
            'iso_639_2'    => 'zho',
            'http_name'    => 'zh-TW',
            'priority'     => 1,
        ],
        'zh'    => [
            'english_name' => 'Chinese',
            'native_name'  => '中文',
            'is_rtl'       => 0,
            'iso_639_1'    => 'zh',
            'iso_639_2'    => 'zho',
            'http_name'    => 'zh',
            'priority'     => 1,
        ],
    ];
}